...
def paperFig2_draw_bc(errors, xvals = None):
    ...
    for h in range(2):
        for i in range(1+len(reacs)):
            ...
            if h ==0 and i >0: ax.set_yticks([10**-(1*i+2) for i in range(3)])
            if h ==1 and i >0: ax.set_yticks([10**-(1*i+1) for i in range(4)])
    ...
    for i in range(3):
        for j in range(len(reacs)):
            for k in range(len(OMEGAS)):
                ...
                # Set axis properties
                if i ==0: axs[i,j].set_yticks(ticks = [0.5, 0.4, 0.3])
                if i ==1: axs[i,j].set_yticks(ticks = [0.05,0.03, 0.01])
                if i ==2: 
...
def paperFigS2_draw(chisN_times_dict, N_dnstimes_dict):
    ...
    coeffs_loglinMPS =  [np.polyfit(xDNS_values, (yMPS_values[c*numN:(c+1)*numN]),  1) for c in range(numChi) ]
    poly_DNS   = np.poly1d(coeffs_loglogDNS)
    poly_MPS   = [np.poly1d(coeffs) for coeffs in coeffs_loglinMPS]
    ...
    y_fit_MPS =[poly_fit(np.log2(x_fit)) for poly_fit in poly_MPS]  # Calculate y-values using the polynomial model
