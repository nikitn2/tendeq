...
def mpsDmrg_autodiff(mps_init, chi_max, loss_fn, loss_kwargs, eps = 1e-15, 
>>>>>>> origin/main
                     optimizer_opts = {"optimizer" : 'l-bfgs-b', "autodiff_backend" : 'jax'}, optimize_opts = {'jac': True, 'hessp':False, 'ftol':1e-12, 'gtol': 1e-12, 'eps' : 1e-5, "n" : 1000}):
...
>>>>>>> origin/main
            ...
            mps_new.expand_bond_dimension(chi_new, rand_strength=1e-14, inplace=True); mps_new.compress(form='right',cutoff_mode = "rel",cutoff=1e-15)
            mps_new.canonize(i)
<<<<<<< HEAD

            # Prep optimiser
            tagsToOpti = [sti.format(j) for j in range(i,min(i+3, mps_new.L))]
            tnopt = qu.tensor.TNOptimizer(mps_new, tags = tagsToOpti, loss_fn=loss_fn, loss_constants=loss_kwargs, **optimizer_opts)
...
def mpsHadamardZipupProd(u1_mps, u2_mpso, mpoContractLegs = "lower", max_intermediateBond = None, max_finalBond = None, tol_bond = 1e-15):
<<<<<<< HEAD
...
>>>>>>> origin/main
            ...
            uC2_mpso = u2_mpso.reindex_lower_sites('hadamard_mpsHadamardZipupProd{}', inplace=False) #<-- This is the index which will be contracted over.
        else:
            remainderIndices = (u2_mpso.upper_ind_id,u2_mpso.lower_ind_id)
            uC2_mpso = u2_mpso.reindex_upper_sites('hadamard_mpsHadamardZipupProd{}', inplace=False) #<-- This is the index which will be contracted over.
<<<<<<< HEAD
...
>>>>>>> origin/main
    for n in range(0,N):
        copyTensor = qu.tensor.tensor_core.COPY_tensor(d=physDim, inds=[contractingIndId.format(n),'temp{}'.format(n),'prelower{}'.format(n)])
        uC_mps &= copyTensor
...
def mpsSqrt_dmrg(mps, chi = None, optimizer_opts = {"optimizer" : 'l-bfgs-b', "autodiff_backend" : 'jax'}, optimize_opts = {'jac': True, 'hessp':False, 'ftol':1e-12, 'gtol': 1e-12, 'eps' : 1e-5, "n" : 1000} ):
...
<<<<<<< HEAD
    ...
    mps_sqrt = mpsDmrg_autodiff(mps_init = mps, chi_max = chi, loss_fn = mpsSqrt_loss, loss_kwargs = {"mps_targ": mps}, optimizer_opts = optimizer_opts, optimize_opts = optimize_opts)
...
def mpsComputeMutualInf(uu_mps):
    ...
    N = uu_mps.L
    mis = np.zeros([N,N])
    uu_mpsC = uu_mps.copy(); uu_mpsC.normalize(); qdat = qu.qu((uu_mpsC ^...).data.transpose().reshape(-1,))
    for i in range (0,N):
        for j in range (0,N):
    ...
    for n in range(0,ND):
        for d in range(0,D):
            if isinstance(uu_mpso,qu.tensor.tensor_1d.MatrixProductState):
                uu_mpso[D*n+d].reindex({uu_mpso.site_ind_id.format(D*n+d) : uu_mpso.site_ind_id.format(n+ND*d)}, inplace=True)
            else:
                uu_mpso[D*n+d] .reindex({ uu_mpso.upper_ind_id.format(D*n+d) : uu_mpso.upper_ind_id.format(n+ND*d)}, inplace=True)
                uu_mpso[D*n+d] .reindex({ uu_mpso.lower_ind_id.format(D*n+d) : uu_mpso.lower_ind_id.format(n+ND*d)}, inplace=True)
    return
...
<<<<<<< HEAD
    ...
    # Now find the best MPS representation that has a bond dimension chi using the method of “alternating least squares”.
    uu_mps_var = uu_mpsExact.copy(); uu_mps_var.compress(max_bond = chi)
    #uu_mps_var = qu.tensor.MPS_rand_state(L=N, bond_dim=1)
...
>>>>>>> origin/main
    ...
    def siteSplitQR(mps, n, siteTagId):
        lsite = siteTagId.format(n-1); csite = siteTagId.format(n)
        lInds   = ['k{}'.format(n)] + [*mps[csite].bonds(mps[lsite])]
        newLTag = set([siteTagId.format(n)]); remTags= set(mps[csite].tags) - newLTag
        mps.split_tensor(tags=mps[csite].tags, left_inds = lInds, ltags = "tempLeft", rtags = "tempRight", method='qr')
        Tl = mps["tempLeft"]; Tl.drop_tags(); Tl.add_tag(newLTag)
        Tr = mps["tempRight"];Tr.drop_tags(); Tr.add_tag(remTags)
        return
<<<<<<< HEAD
...
>>>>>>> origin/main
    ...
    prolo_mps.reindex_all('k{}', inplace=True); prolo_mps.retag_all(orig_mps.site_tag_id, inplace=True)
<<<<<<< HEAD
...
>>>>>>> origin/main
    ...
    if split == False:
        ...
        prolo_mps.contract_tags(specialSites, inplace=True); prolo_mps.fuse_multibonds(inplace=True)
        #
...
>>>>>>> origin/main
    ...
    if isinstance(mpso, qu.tensor.tensor_1d.MatrixProductState): physIndId = (mpso.site_ind_id,)
    else: physIndId = (mpso.lower_ind_id, mpso.upper_ind_id)
...
<<<<<<< HEAD
    ...
    leftOrigTags  = mpso[n-1].tags; lTen.drop_tags(); lTen.add_tag("tocontr")
    rightOrigTags = mpso[n].tags;   rTen.drop_tags(); rTen.add_tag("tocontr")
...
>>>>>>> origin/main
    ...
    mpso.split_tensor( tags="tocontr", left_inds = leftInds, method='svd', absorb = "left", max_bond=max_bond, cutoff=cutoff, ltags=leftOrigTags,rtags=rightOrigTags)
...
>>>>>>> origin/main
    # Compress and finish.
    mpso.compress(**COMP_OPTS); mpso.compress(max_bond = max_bond)
...
>>>>>>> origin/main
    # Seperate time/spatial dimensions requires special consideration
        ...
        if Nt >= NK: mpsoI = mpsoInterleave(mpso = mpsoI, K = K+1, NK =  NK, max_bond=None, cutoff=cutoff)
        else: print("Nt<NK not currently supported. Exiting.\n"); sys.exit(1)
...
>>>>>>> origin/main
    ...
    # Seperate time/spatial dimensions requires special consideration
        ...
        if Nt >= NK: mpsoRI = mpsoRevInterleave(mpso, K+1, NK, max_bond=None, cutoff=cutoff)
        else: print("Nt<NK not currently supported. Exiting.\n"); sys.exit(1)
...
def shiftIndextagMps(mpsOrig, Nextra, start = 0, shifted_ind_id = None, tag_shift = True, N = None):
    ...
    if isinstance(mpsOrig, qu.tensor.tensor_1d.MatrixProductState):
        ...
        else: siteIndId=(shifted_ind_id,)
    else:
        if shifted_ind_id==None: siteIndId = (mpsOrig.lower_ind_id, mpsOrig.upper_ind_id)
        else: siteIndId = (shifted_ind_id["lower"], shifted_ind_id["upper"])
    ...
    if shifted_ind_id != None:
        if isinstance(mpsOrig, qu.tensor.tensor_1d.MatrixProductState): mpsOrig.reindex_all(shifted_ind_id, inplace=True)
        else:
...
=======
def shiftIndextagMps(mpsOrig, Nextra):
    
    # Initialise
    N = mpsOrig.L
    siteTagId = mpsOrig.site_tag_id
    if isinstance(mpsOrig, qu.tensor.tensor_1d.MatrixProductState): siteIndId = (mpsOrig.site_ind_id,)
    else: siteIndId = (mpsOrig.lower_ind_id, mpsOrig.upper_ind_id)
    ...
    mpsNew=mpsOrig.reindex(reindexSiteIndMap,inplace=False)
    mpsNew.retag(          reindexSiteTagMap,inplace=True )
...
<<<<<<< HEAD
    ...
    # Do a few prechecks
    if mps1 is None: return mps2
    elif mps2 is None: return mps1
...
>>>>>>> origin/main
    if K_left > 0:
        uniSpace_mps = qu.tensor.tensor_builder.MPS_product_state(arrays = [np.array([1.0,1.0])]*NK*K_left,site_tag_id=siteTagId, site_ind_id=siteIndId)
        lin_mps = shiftIndextagMps(lin_mps, Nextra = NK*K_left)
        lin_mps &= uniSpace_mps
        qu.tensor.tensor_core.new_bond( lin_mps[siteTagId.format(NK*K_left -1)], lin_mps[siteTagId.format(NK*K_left)] ); lin_mps._L += NK*K_left
<<<<<<< HEAD
...
>>>>>>> origin/main
    if K_right>0:
        uniSpace_mps = qu.tensor.tensor_builder.MPS_product_state(arrays = [np.array([1.0,1.0])]*NK*K_right,site_tag_id=siteTagId, site_ind_id=siteIndId)
        uniSpace_mps = shiftIndextagMps(uniSpace_mps, Nextra = NK*(K_left+1) )
        lin_mps &= uniSpace_mps
        qu.tensor.tensor_core.new_bond( lin_mps[siteTagId.format(NK*(K_left+1)-1)], lin_mps[siteTagId.format(NK*(K_left+1))] ); lin_mps._L += NK*K_right
...
<<<<<<< HEAD
    ...
    uni_mps = qu.tensor.tensor_builder.MPS_product_state(arrays = [np.array([1,1])]*N,site_tag_id=siteTagId, site_ind_id=siteIndId)
...
<<<<<<< HEAD
        ...
        finTen = mpsAvgd[N-1]; finTen.drop_tags(); finTen.add_tag("s{}".format(NK*(K-KtoAvg) -1))
...
>>>>>>> origin/main
        ...
        if padBack is True:
            mpsAvgd &= flat_mps
            qu.tensor.tensor_core.new_bond( mpsAvgd["s{}".format( NK*(K-KtoAvg)-1 )], mpsAvgd["s{}".format( NK*(K-KtoAvg) )] )
            mpsAvgd._L = N
<<<<<<< HEAD
...
>>>>>>> origin/main
        ...
        if Dim == 0: absrbTag = "s{}".format(NK*(Dim+1))
        else: absrbTag = "s{}".format(NK*Dim-1)
        contrTags += [absrbTag]
    mpsInteg  = mpsInteg.contract(tags=contrTags)
<<<<<<< HEAD

    # Retag the integral-containing tensor
    integTen = mpsInteg[NK*Dim]; integTen.drop_tags(); integTen.add_tag(absrbTag)
...
>>>>>>> origin/main
    for k in range(0,Ktot):
        for n in range(0,NK):
            ...
            if(split==True): tensorNum = k*NK + n
            # In case an interleaved ordering of the MPS chain:
            else: tensorNum = n*Ktot + k
<<<<<<< HEAD

            if k == K: tag = "t_{}".format(n)
            else: tag = 'x{}_{}'.format(k+1,n)
...
>>>>>>> origin/main
    ...
    while sum(len(prime) for prime in primes) > 0:
        ...
        for prime in primes:
            if len(prime)==0: k+=1
            elif len(prime)>0: ks_valid.append(k); prime.pop(0); k+=1
...
>>>>>>> origin/main
    ...
    old_inds = np.arange(len(dims)); new_inds = 0*old_inds.copy()
    it = 0
    for n in range(max(NKs)):
        for k in Ks_valid[n]:
            #print("n={}".format(n),"k={}".format(k), ":" , it, " ->", n + np.sum(NKs[:k],dtype=int))
            if split == True: new_inds[it] = old_inds[it]
            else: new_inds[it] = n + np.sum(NKs[:k],dtype=int)
            it+=1
<<<<<<< HEAD
...
def mpsGetFidelityCompRatio(u, split, chi, cutoff):
    ...
    u_mps_compr = u_mps.copy(); u_mps_compr.compress(max_bond=chi, cutoff=cutoff)
...
=======
    ...
    if split==True:
        for k in range(max(KNs)):
            for n in range(NKs[k]): fuse_map['F{}'.format(k)].append( "k{}".format(it) ); it+=1
    else:
        for n in range(max(NKs)):
            for k in Ks_valid[n]: fuse_map['F{}'.format(k)].append( "k{}".format(it) ); it+=1
...
def mpoCreateAcc1TemporalDiff(N, NpadLeft = 0, NpadRight=0, siteTagId='s{}',temporalTensorsTag='t', upIndIds="k{}", downIndIds="b{}", split=True, **splitOpts):
<<<<<<< HEAD
...
<<<<<<< HEAD
    ...
    # Now produce the MPO
    diff_tn = qu.tensor.tensor_1d.MatrixProductOperator(arrays= [idLeft_tensor.data]*int(NpadLeft>0) + [idBulk_tensor.data for _ in range(NpadLeft-1)] + [left_tensor.data] + [bulk_tensor.data for _ in range(N-2)] + [right_tensor.data] + [idBulk_tensor.data for _ in range(NpadRight-1)] + [idRight_tensor.data]*int(NpadRight>0),
...
def mpoCreatePlusShift1(N, boundryCond = "periodic", NpadLeft = 0, NpadRight=0, siteTagId='s{}', upIndIds="k{}", downIndIds="b{}", split = True, **splitOpts):
<<<<<<< HEAD
    ...
    if boundryCond == "periodic": left_array[1 + 2*(0) + 4*(1) ] = 1.0
    elif boundryCond == "dirichlet": pass# or boundryCond == "normconserving": pass# do nothing
    else: print("The provided boundary conditions are not currently supported. Exiting.\n"); sys.exit(1)
    ...
    id_array_chi1[0+2*0] = 1.0; id_array_chi1[1+2*1] = 1.0
...
<<<<<<< HEAD
    ...
    arrays = [idLeft_tensor.data]*int(NpadLeft>0) + [idBulkChi1_tensor.data]*(NpadLeft-1) + [left_tensor.data] + [bulk_tensor.data]*(N-2) + [right_tensor.data] + [idBulkChi1_tensor.data]*(NpadRight-1) + [idRight_tensor.data]*int(NpadRight>0)

    # Now finally form the MPO
    PlusShift1_tn = qu.tensor.tensor_1d.MatrixProductOperator(arrays= arrays, shape='dlur', site_tag_id=siteTagId, upper_ind_id=upIndIds, lower_ind_id=downIndIds)#, bond_name='internal')
...
def mpoCreateAcc8SpatialDiff(N, diffOrder, boundryCond = "periodic", NpadLeft = 0, NpadRight=0, h=1.0, siteTagId='s{}', upIndIds="k{}", downIndIds="b{}", split = True, **splitOpts):
<<<<<<< HEAD
...
def mpoCreateAcc2SpatialDiff(N, diffOrder, NpadLeft = 0, NpadRight=0, h=1.0, boundryCond="periodic", siteTagId='s{}', upIndIds="k{}", downIndIds="b{}", split = True, **splitOpts):
    ...
    # Create the building blocks
    if boundryCond == "normconserving" or boundryCond == "zerograd":  S_p1 = mpoCreatePlusShift1(N, "dirichlet", NpadLeft, NpadRight, siteTagId, upIndIds, downIndIds)
    else: S_p1 = mpoCreatePlusShift1(N, boundryCond, NpadLeft, NpadRight, siteTagId, upIndIds, downIndIds)
...
<<<<<<< HEAD
    ...
    else: print("only diffOrder = 1,2 currently supported. Exiting.\n"); sys.exit(1)
...
>>>>>>> origin/main
    ...
    if boundryCond == "normconserving":
        delta0, deltaF = create_deltas0andF(N, NpadLeft = NpadLeft, NpadRight=NpadRight, siteTagId = siteTagId, siteIndId= upIndIds)
        if diffOrder == 1:
...
<<<<<<< HEAD
    ...
    uni    = qu.tensor.tensor_builder.MPS_product_state(arrays = [np.array([1,1]) for _ in range(NpadLeft+N+NpadRight)], site_tag_id=siteTagId, site_ind_id=siteIndId)
...
<<<<<<< HEAD
    ...
    delta0 = qu.tensor.tensor_builder.MPS_product_state(arrays = [np.array([1.0,1.0]) for _ in range(NpadLeft)] + [np.array([1.0,1e-40]) for _ in range(N)] + [np.array([1.0,1.0]) for _ in range(NpadRight)], site_tag_id=siteTagId, site_ind_id=siteIndId)
    deltaF = qu.tensor.tensor_builder.MPS_product_state(arrays = [np.array([1.0,1.0]) for _ in range(NpadLeft)] + [np.array([1e-40,1.0]) for _ in range(N)] + [np.array([1.0,1.0]) for _ in range(NpadRight)], site_tag_id=siteTagId, site_ind_id=siteIndId)
...
def mpoCreateAcc2Prolongator1D(N, boundryCond = "periodic", NpadLeft = 0, NpadRight=0, siteTagId='s{}', tensorTag=[], upIndIds="k{}", downIndIds="b{}"):
<<<<<<< HEAD
    ...
    # If reached boundary, act
    if   boundryCond == "periodic":    left_array[0 + 2*(1) + 4*(1) ] = 1.0
    elif boundryCond == "dirichlet": pass# do nothing
    else: print("The provided boundary conditions <{}> are not currently supported. Exiting.\n".format(boundryCond)); sys.exit(1)
...
<<<<<<< HEAD
    ...
    # Now produce the MPO
    prolo_tn = qu.tensor.tensor_1d.MatrixProductOperator(arrays= [idLeft_tensor.data]*int(NpadLeft>0) + [idBulk_tensor.data for _ in range(NpadLeft-1)] + [left_tensor.data] + [bulk_tensor.data for _ in range(N-1)] + [right_tensor.data] + [idBulk_tensor.data for _ in range(NpadRight-1)] + [idRight_tensor.data]*int(NpadRight>0),
                                                        shape='dlur', site_tag_id=siteTagId, upper_ind_id=upIndIds, lower_ind_id=downIndIds, bond_name='internal')
...
# KD Prolongator
def mpoCreateAcc2ProlongatorKD(K, NK, Nt, boundryCond= "periodic", siteTagId='s{}', tensorTag=[], upIndIds="k{}", downIndIds="b{}", split = True):
<<<<<<< HEAD
...
>>>>>>> origin/main
    ...
    reindexSiteIndMap = {}
    if split == True:
        for n in range(0, K*(NK+1)):         reindexSiteIndMap[downIndIds.format(n)] = downIndIds.format(int(n - n//(NK+1)))
